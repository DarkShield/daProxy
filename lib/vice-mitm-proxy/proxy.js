...
var handle_request = function(that, request, response, type) {
  ...
  if(that.options.verbose) console.log( 'proxying to ' + request.url);
  ...
  request.on('close', function() {
    ...
    else{ sys.log('Request Connection Undefined'); }
  })
...
module.exports = function(proxy_options, processor_class) {
  ...
  var server = http.createServer(function(request, response) {
    ...
    if (request.headers.host && proxy_options.hosts[request.headers.host.replace(/\./g, "")] === 'enabled') {
      ...
      response.end();
      sys.log('Request Aborted, not proxied');
    }
  });
  ...
  this.startServer = function(){
    ...
    if(this.options.verbose) sys.log('https man-in-the-middle proxy server started on port ' + this.options.mitm_port);
    server.listen(this.options.proxy_port);
    if(this.options.verbose) sys.log('http proxy server started on port ' + this.options.proxy_port);
  };
